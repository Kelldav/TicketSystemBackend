...
public class UpdateFiles {
    ...
    Boolean writeDTF() {
        ...
        try {
            ...
            for (Transaction transaction : transactionList) {
                if (transaction.code.equals("01") || transaction.code.equals("02") || transaction.code.equals("06")
                        || transaction.code.equals("00")) {
                    transactionStr = transaction.code + paddingSpace("name", transaction.username)
                            + transaction.userType + paddingZero("credit", transaction.credit, 0);
                } else if (transaction.code.equals("05")) {
                    transactionStr = transaction.code + paddingSpace("name", transaction.username)
                            + paddingSpace("name", transaction.seller) + paddingZero("credit", transaction.credit, 0);
                } else if (transaction.code.equals("04") || transaction.code.equals("04")) {
                    transactionStr = transaction.code + paddingSpace("event", transaction.eventName)
                            + paddingSpace("name", transaction.seller)
                            + paddingZero("tickets", 0, transaction.ticketAmount)
                            + paddingZero("price", transaction.ticketPrice, 0);
                }
    ...
    Boolean writeEventsFile() {
        ...
        try {
            ...
            for (Event event : eventList) {
                eventStr = paddingSpace("event", event.getEventName()) + paddingSpace("name", event.getSellerUsername())
                        + paddingZero("tickets", 0, event.getNumOfTickets())
                        + paddingZero("price", event.getTicketPrice(), 0);
    ...
    Boolean writeUserAccountsFile() {
        ...
        try {
            ...
            for (User user : userList) {
                userStr = paddingSpace("name", user.getUsername()) + user.getAccountType()
                        + paddingZero("credit", user.getCredit(), 0);
    ...
    /**
     * This method adds padding (zeros) to credit, number of tickets, or ticket
     * price strings for writing to the daily transaction file.
     * 
    ...
    /**
     * This method adds padding spaces (_) to usernames, or event names for writing
